...
rides.groupby("workingday", as_index=False)["cnt","casual","registered"].sum().plot(kind="bar", x='workingday', y=['casual','registered','cnt'])


# In[8]:


rides.groupby("holiday", as_index=False)["cnt","casual","registered"].sum().plot(kind="bar", x='holiday', y=['casual','registered','cnt'])
...
data.loc[:, ['casual', 'registered', 'cnt', 'temp', 'hum', 'windspeed']].head()
...
class NeuralNetwork(object):
    ...
    def train(self, features, targets):
        ...
        for X, y in zip(features, targets):
            ...
            hidden_error = error.dot(self.weights_hidden_to_output.T) * self.activation_function(hidden_inputs) * (1 - self.activation_function(hidden_inputs))
...
predictions = np.array([network.run(test_features)])
predictions = predictions.T*std + mean
ax.plot(predictions.T[0], label='Prediction')
